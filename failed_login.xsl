			...
			<head>
				...
				<title>
					...
					<xsl:call-template name="formatted_name">
						<xsl:with-param name="final_text" select="'Login Page'"/>
					</xsl:call-template>
				</title>
				...
				<link href="/login_page_layout.css" rel="stylesheet" type="text/css" />
			</head>
			<body>
				...
				<xsl:comment>Form for obtaining login information</xsl:comment>
				<div id="main_content">
					<xsl:comment>LXI logo</xsl:comment>
					<img id="lxi_logo" src="/Images/lxi_logo.png" alt="LXI"/>
					<div id="form_data">
						<form method="POST" action="login_attempt.cgi">
							...
							<input id="submit_button" value="Login" type="submit"/>
						</form>
						<p id="login_status">
							Login failed.  Either the user id or password was incorrect.
						</p>
					</div>
				</div>
